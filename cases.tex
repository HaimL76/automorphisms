...
There are no two sub cases here, because the phase transition occurs at $v_{11}={v_4-v_1-v_2-1}$, thus if $v_4=0$ then the transition would be at $v_{11}=-v_2-v_1-1<-v_2-v_1$, which is a contradiction. For $v_4=1$ the transition would be at $a_{11}=-v_2-v_1$, but then $\alpha=0=v_4$ so there is no transition as well. For $v_4>1$ we have $\alpha<v_4$.
