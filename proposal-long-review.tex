...
\end{proposition}
This proposition gives rise to an entire subject in group theory, called \textbf{subgroup growth}. We denote by $a_{n}(G):=|\{H\leq{G} : [G:H]=n\}|$ the number of subgroups of $G$ of index $n$, and look at the sequence $\{a_n(G)\}_{n=1}^{\infty}$. The subject of subgroup growth aims to relate the properties of this sequence to the algebraic structure of $G$. We denote by $a_{n}^{\trianglelefteq}:=|\{H\trianglelefteq{G} : [G:H]=n\}|$ the number of \textbf{normal} subgroups of $G$. Now we define another type of subgroups.
\begin{definition}
...
Let $G$ be any group, and let \[\hat{a_n}(G):=|\{H\leq G : \widehat{H}\cong\widehat{G}, [G:H]=n\}|\] be the number of pro-isomorphic subgroups of $G$ of index $n$. Assume that $\hat{a_n}(G)<\infty$ for all $n$.
\end{definition}




For instance, Lubotzky, Mann and Segal showed in \cite{LubotzkyMannSegal} that $a_n(G)$ grows polynomially if and only if $G$ is virtually solvable of finite rank, that is, $G$ has a finite-index solvable subgroup, and all finitely-generated subgroups may be generated by a bounded number of generators. This research concentrates on the growth of \textbf{pro-isomorphic} subgroups, which we now define.
...
be the diagonal $m\times m$ matrix where $m=\binom{n}{2}$, of the same form of $h$ from above for $n-1$ scalars, then $\det A_n=\prod_{i=1}^{n-1}\lambda_i^{i(n-i)}$.
\end{proposition}
\begin{proof}
