...
\item There is an automoprhism $\varphi\in{G_n(\mathbb{Q}_{p})}$ determined by \[
    \varphi(e_{i,i+1})=\begin{cases}
        e_{k,k+1}+a{e_{k,k+r}} & : i=k\\
        e_{k+r,k+r+1}-a{e_{k+1,k+r+1}} & : i=k+r\\
        e_{i,i+1} & : i\notin\{k,k+r\}
    \end{cases}
\]
\item Suppose that $r=n-2$, let $(k,l)\in\{(1,2),(n-1,1)\}$, and let $a\in\mathbb{Q}_{p}$. There is an automorphism $\varphi\in{G_n(\mathbb{Q}_{p}}$ determined by \[
    \varphi(e_{i,i+1})=\begin{cases}
