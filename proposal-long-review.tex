...
\begin{enumerate}
    \item Let $2\leq{r}\leq{n-3}$. If $a_{ij}=0$, then $i=j$ or $i=j+r-1$.
    \item Let $r=n-2$. If $a_{ij}=0$, then $i=j$ or $i=j+r-1$ or $(i,j)\in\{(1,2),(n-1,1)\}$.
\end{enumerate}
\end{proposition}
\begin{proof}
From the relation $[\varphi(e_{k,k+1}),\varphi(e_{l,l+1})]=0$ where $l>k+1$, we deduce that $a_{ij}\neq{0}$ only if either $i=j$ or $i=j+r-1$ or $(i,j)\in\{(r+1,1),(r+2,1),(n-r-2,n-r),(n-r-1,n-r)\}$.
\end{proof}
