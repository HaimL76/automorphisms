...
For all $2\leq{r}\leq{n-1}$ we denote by $N_{r}\subset{N(\mathbb{Q}_{p})}$ the subset of all automorphisms of $\mathcal{L}_{n,p}$, such that $M_{11}=I_{n}$ and $M_{12}=M_{13}=\cdots=M_{1,r-1}=0$. We check that $N_{r}$ is a subgroup of $N(\mathbb{Q}_{p})$. It is easy to see that \[
...
\]
for $h>0$.
Assume that \[[\varphi_{r,k}(a)(e_{ij}),\varphi_{r,k}(a)(e_{lm})]=\]\[=[[\varphi_{r,k}(a)(e_{i,i+1}),\varphi_{r,k}(a)(e_{i+1,j})],\varphi_{r,k}(a)(e_{l,m})]=\cdots=\]\[=\cdots=[[\varphi_{r,k}(a)(e_{ij}),[\varphi_{r,k}(a)(e_{l,m-1}),\varphi_{r,k}(a)(e_{m-1,m})]]=\cdots=\]\[=\cdots=\varphi_{r,k}(a)[e_{ij},e_{lm}]\]
for all $i<j$ and $l<m$. In words, $\varphi_{r,k}(a)$ is compatible with the Lie bracket, regardless of how we decompose $e_{ij}$ and $e_{lm}$. Indeed, for $n=4$ and $r=2$, we have only one way to decompose $e_{i,i+2}=[e_{i,i+1},e_{i+1,i+2}]$. For any $n>4$, we define 
\end{proof}
