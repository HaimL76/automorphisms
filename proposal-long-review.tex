...
\end{theorem}
This theorem is known for any $\mathbb{Z}_{p}$-Lie algebra $L_{p}$. We aim to show, for a specific type of $\mathbb{Z}_{p}$-Lie algebras, that it is either uniform or finitely-uniform, which means that the local zeta-functions are represented by a finite set of $r$ rational functions, where $r\geq{1}$, for all but finitely many $p$.
