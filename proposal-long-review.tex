...
We have made progress towards step 1 of our research program, namely determining the automorphism groups $G_{n}(\mathbb{Q}_p)$.
Define an involution $\eta\in{G_{n}(\mathbb{Q}_p)}$ by $\eta(e_{ij}):=(-1)^{j-i-1}e_{n+1-j,n+1-i}$, for $1\leq{i}\leq{n-1}$ and ${i+1}\leq{j}\leq{n}$. Replacing $\varphi$ by $\varphi\circ\eta$, we assume without loss of generality that $M_{11}$ is diagonal.
