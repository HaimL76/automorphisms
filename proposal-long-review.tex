...
\end{definition}
Let $G$ be a $\mathcal{T}$-group, then $\zeta_{G,p}^{\leq}(s)$ satisfies a functional equation, for all but finitely many $p$, with the same symmetry factor; see \cite[Theorem B]{Voll}. If $G$ is a $\mathcal{T}$-group of nilpotency class $2$, then the same is true for $\zeta_{G,p}^{\trianglelefteq}(s)$; see \cite[Theorem C]{Voll}.
