...
We aim to compute an explicit formula for $\hat{\zeta_{L_{n,p}}}(s)$ for all $n \geq 4$ and all primes $p$ in terms of a suitable combinatorial framework that is compatible with the complicated structure of $G^+(\mathbb{Q}_p)$.  As a consequence, we will study functional equations.  In the event that we do not succeed in this task in general, we still expect to complete Berman's unfinished calculation in the case $n=5$.\par
