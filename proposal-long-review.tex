...
There are specific examples of $\mathcal{T}$-groups of nilpotency class $3$, such that $\zeta_{G,p}^{\trianglelefteq}(s)$ is \textbf{uniform}, i.e. $\zeta_{G,p}^{\trianglelefteq}(s)=W(p,p^{-s})$ for all primes $p$, but $W(p,p^{-s})$ does not satisfy a functional equation, see \cite[Theorem 2.40]{DuSautoyWoodward} and \cite[Theorem 2.44]{DuSautoyWoodward}.
