...
The second case is when $\beta=\min\{v_{4},v_{1}+v_{2}+v_{11}\}=v_{1}+v_{2}+v_{11}\Rightarrow{v_{1}+v_{2}+v_{11}<v_{4}}$, we use a strong inequality here, because we have already counted the case where $v_{4}=v_{1}+v_{2}+v_{11}$. But $v_{11}\geq{-(v_{2}+v_{3})}\Rightarrow{v_{11}+v_{2}\geq{-v_{3}}}$, hence $v_{1}-v_{3}\leq{\beta}\leq{v_{4}-1}$. For this case, the expression $\beta(1-p^{-1})$ is not constant, but a finite number of constant values, where $\beta\in\{v_{1}-v_{3},v_{1}-v_{3}+1,v_{1}-v_{3}+2,\dots,v_{4}-2,v_{4}-1\}$.

Comparing the two possible ranges of $v_{11}$, we observe that the lower bound is $-(v_2+v_3)$ in both cases, but the upper bound is either $-(v_2+1)$ coming from $\min\{0,v_2+v_{11}\}$, or $v_4-1-(v_1+v_2)=v_4-v_1-(v_2+1)$ coming from $\beta$, and since $v_4-v_1\leq{0}$, we have that $v_4-v_1-(v_2+1)\leq{-(v_2+1)}$, hence the range of $v_{11}$, for this particular subcase, must be $-(v_2+v_3)\leq{v_{11}}\leq{v_4-v_1}-(v_2-1)$, and therefore 




$\displaystyle\int_{\underline\lambda}I_{5}\displaystyle\int_{a_{11}}p^{\min\{0,v_{2}+v_{11}\}}[1+\beta(1-p^{-1})]d\mu(a_{11})=$
