...
or \[\mathrm{\circled{3}}{\rho}e_{i,i+k+3}=a_{i,i+1+\beta}^{i}e_{i,i+1+\beta}a_{i+1+\beta,i+k+2-\beta}^{i+1}e_{i+1+\beta,i+k+2-\beta}a_{i+k+2-\beta,i+k+3}^{i}e_{i+k+2-\beta,i+k+3}\]
because for any other permutation of , the 




\[{\tau}e_{i,i+2+k'}=a_{i,i+m}^{i+1}e_{i,i+m}a_{i+m,i+m+d}^{i}e_{i+r,i+m+d}a_{i+m+d,i+2+k'}^{i}e_{i+m+d,i+2+k'}\]
for some $2\leq m\leq k'$ and $1\leq d\leq k'+1-m$,
but 
\end{proof}
\end{document}