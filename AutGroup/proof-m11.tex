...
But since $a_{i+j',i+j'+l}^{i},a_{i+j'+l,i+j'+k}^{i}\in\gamma_{k-1}\mathcal{L}_{n,p}$, then by the assumption, we have that $a_{i+j',i+j'+l}^{i}=a_{i+j'+l,i+j'+k}^{i}=0$, for all $2\leq{l}\leq{k-1}$, hence our equation becomes $a_{i+j',i+j'+1}^{i}b_{i+j'+1,i+j'+k}^{i}-a_{i+j'+1,i+j'+k}^{i}b_{i+j',i+j'+1}^{i}e_{i+j'+1,i+j'+k}=0$.
\end{proof}
\end{document}