...
which means that $\mu_{i-1}^i=0$, for every $2\leq i\leq n-1$ and $\mu_{i+1}^i=0$, for every $1\leq i\leq n-2$. We have already seen that besides these constraints, there are no other constraints on $\mathcal{C}_{\sfrac{\gamma_1}{\gamma_3}}(\varphi(e_{i,i+1}))$
