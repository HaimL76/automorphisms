...
\end{proposition}
\begin{proof}
We first observe that for every $2\leq i\leq n-2$, if $y=\sum_{j=1}^{n-1}\mu_j^i e_{j,j+1}+z_2\in\mathcal{C}_{\sfrac{\gamma_1}{\gamma_3}}(\varphi(e_{i,i+1}))$, then we have the set of equations $\{\lambda_i\mu_{i+1}^i e_{i,i+2}=0,-\lambda_i\mu_{i-1}^i e_{i-1,i+1}=0\}_{i=2}^{n-2}$, hence $\mu_{i-1}^i=\mu_{i+1}^i=0$, for every $2\leq i\leq n-2$. In addition, we have that $\lambda_1\mu_2^1 e_{13}=0$ and $-\lambda_{n-1}\mu_{n-2}^{n-1}e_{n-2,n}=0$, hence $\mu_2^1=0$ and $\mu_{n-2}^{n-1}=0$.




For $\varphi(e_{12})$ we have the equation $\lambda_2$
...
We first observe that for every $1\leq i\leq n-2$, we have that \[[\varphi(e_{i,i+1}),\varphi(e_{i,i+2})]=[\lambda_i e_{i,i+1}+a_{i-2,i}^{i,i+1}e_{i-2,i}+a_{i+2,i+4}^{i,i+1}e_{i+2,i+4}+u_2^i,\lambda_i\lambda_{i+1}e_{i,i+2}+v_2^i]\]\[=0=a_{i-2,i}^{i,i+1}\lambda_i\lambda_{i+1}e_{i-2,i+2}-a_{i+2,i+4}^{i,i+1}\lambda_i\lambda_{i+1}e_{i,i+4}+w_3^i\] and \[[\varphi(e_{i+1,i+2}),\varphi(e_{i,i+2})]=[\lambda_{i+1} e_{i+1,i+2}+a_{i-2,i}^{i+1,i+2}e_{i-2,i}+a_{i+2,i+4}^{i+1,i+2}e_{i+2,i+4}+u_2^{i+1},\lambda_i\lambda_{i+1}e_{i,i+2}+v_2^{i+1}]\]\[=0=a_{i-2,i}^{i,i+1}\lambda_i\lambda_{i+1}e_{i-2,i+2}-a_{i+2,i+4}^{i,i+1}\lambda_i\lambda_{i+1}e_{i,i+4}+w_3^{i+1}\]
\end{proof}
